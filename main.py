...
# Multi-value column expansion util (inline as requested)
# def load_multi_value_expansion(json_path: str) -> dict:
#     with open(json_path, "r", encoding="utf-8") as f:
#         config = json.load(f)
#     return config.get("multi_value_column_expansion", {})

# def expand_multi_value_columns_api(data: dict, multi_value_expansion: dict) -> dict:
#     expanded = data.copy()
#     for col, uniqs in multi_value_expansion.items():
#         if col not in expanded:
#             continue
#         vals = re.split(r'\s*,\s*|\s+and\s+', str(expanded[col]))
#         vals = [v.strip() for v in vals if v.strip()]
#         for u in uniqs:
#             expanded[u] = int(u in vals)
#         del expanded[col]
#     return expanded
...
def expand_multi_value_columns_api(data: dict, multi_value_expansion: dict) -> dict:
    """
    Expands multi-value columns as <col>__<unique_value>
    and removes the original multi-value column.
    """
    expanded = data.copy()
    for col, uniqs in multi_value_expansion.items():
        if col not in expanded:
        ...
        vals = [v.strip() for v in vals if v.strip()]
        for u in uniqs:
            # Always use <col>__<unique_value> as column name
            new_col = f"{col}__{u}"
            expanded[new_col] = int(u in vals)
        del expanded[col]
    return expanded

# Load model configuration
MODEL_CONFIG = load_model_config()
MULTI_VALUE_EXPANSION = load_multi_value_expansion('Mode_of_delivery2.json')
...
def safe_load_model(model_path: str, model_name: str):
    ...
    for method_name, load_func in loading_methods:
        try:
            ...
            if method_name == loading_methods[-1][0]:
                raise e
            continue
    raise Exception(f"All loading methods failed for {model_name}")
...
def load_models():
    ...
    for model_name, config in MODEL_CONFIG.items():
        try:
            ...
            model = safe_load_model(model_path, model_name)
            loaded_models[model_name] = model
            model_is_pipeline[model_name] = isinstance(model, Pipeline)
        except Exception as e:
            logger.error(f"Error loading model {model_name}: {str(e)}")
            failed_models.append(f"{model_name}: {str(e)}")
            continue
...
def validate_and_clean_data_for_model(data: Dict[str, Any], model_name: str) -> pd.DataFrame:
    try:
        # ----- Multi-value column expansion -----
        data = expand_multi_value_columns_api(data, MULTI_VALUE_EXPANSION)
        # ----------------------------------------
        df = pd.DataFrame([data])
        if model_name not in MODEL_CONFIG:
            raise HTTPException(status_code=400, detail=f"Model {model_name} not found in configuration")
        ...
        for col in df.columns:
            ...
            if is_numeric_column:
                if 'apgar' in col.lower():
                    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(7.0)
                else:
                    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0.0)
            else:
                df[col] = df[col].astype(str).str.strip()
