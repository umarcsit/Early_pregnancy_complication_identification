...
def load_model_config():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Invalid JSON in configuration file: {str(e)}"
        )
...
def safe_load_model(model_path: str, model_name: str):
    ...
    loading_methods = [
        (
            ...
            lambda: _load_with_sklearn_compatibility_fix(normalized_path),
        ),
        (
            "joblib with warnings suppressed",
            lambda: _load_with_suppressed_warnings(normalized_path),
        ),
        (
            "joblib with custom protocol",
            lambda: _load_with_custom_protocol(normalized_path),
        ),
        (
            "pickle with protocol 4",
            lambda: _load_with_pickle_protocol(normalized_path, 4),
        ),
        (
            "pickle with protocol 5",
            lambda: _load_with_pickle_protocol(normalized_path, 5),
        ),
    ]
...
def _create_mock_model(model_name: str):
    ...
    mock_pipeline = Pipeline(
        [
            ...
            (
                "classifier",
                DummyClassifier(strategy="constant", constant="mock_prediction"),
            ),
        ]
    )
...
def apply_same_preprocessing_as_training(X):
    ...
    try:
        ...
        for col in X_processed.columns:
            ...
            X_processed[col] = X_processed[col].replace(
                ["nan", "none", "null", "", "na", "NaN", "None", "NULL"],
                "not specified",
            )
    ...
    try:
        ...
        for col in df.columns:
            ...
            df[col] = df[col].replace(
                ["nan", "none", "null", "", "na", "NaN", "None", "NULL", "nan", "None"],
                "not specified",
            )
...
def get_missing_features(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if model_name not in MODEL_CONFIG:
            raise HTTPException(
                status_code=500, detail=f"Model {model_name} not found in configuration"
            )
        ...
        return {
            ...
            "completeness_percentage": round(
                (len(provided_cols) / len(required_cols)) * 100, 2
            ),
        }
        ...
        raise HTTPException(
            status_code=400, detail=f"Error checking missing features: {str(e)}"
        )


def validate_and_clean_data_for_model(
    ...
    try:
        ...
        if model_name not in MODEL_CONFIG:
            raise HTTPException(
                status_code=400, detail=f"Model {model_name} not found in configuration"
            )
        ...
        for col in df.columns:
            ...
            if is_numeric_column:
                ...
                if col == "Postnatal_Symptoms" and df[col].iloc[0] == "not specified":
                    ...
                    col == "Postnatal_Examination"
                    and df[col].iloc[0] == "not specified"
                ):
                    df[col] = "normal"
        ...
        raise HTTPException(
            status_code=400, detail=f"Data validation error for {model_name}: {str(e)}"
        )


def get_missing_features_for_model(
    data: Dict[str, Any], model_name: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Error checking missing features for {model_name}: {str(e)}",
        )
...
def make_prediction(model, data: pd.DataFrame, model_name: str) -> tuple:
    ...
    try:
        ...
        if hasattr(model, "classes_"):
            ...
            prob_dict = {
                str(cls): float(prob) for cls, prob in zip(classes, prob_values)
            }
        ...
        raise HTTPException(
            status_code=500, detail=f"Prediction failed for {model_name}: {str(e)}"
        )
    ...
    try:
        ...
        for i, model_name in enumerate(MODEL_ORDER):
            ...
            prediction, prob_dict = make_prediction(
                loaded_models[model_name], model_input, model_name
            )
        ...
        data = {
            "predictions": {
                ...
                "Antenatal_Peripartum_Maternal_Complications": results[
                    "Antenatal_Peripartum_Maternal_Complications"
                ],
            },
            "endpoint": "modeofdelivery_antinatal"
        }
...
async def predict_antenatal_complications(data: PatientData):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in antenatal complications prediction: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
...
async def predict_neonatal_complications(data: PatientData):
    ...
    try:
        ...
        data = {
            "predictions": {
                "Neonatal_Fetal_Complications": prediction
            },
            "endpoint": "neonatal"
        }
        response = requests.post("http://127.0.0.1:8002/generate-remarks", json=data)
...
async def predict_postnatal_complications(data: PatientData):
    ...
    try:
        ...
        data = {
            "predictions": {
                "Postnatal_Maternal_Complications": prediction
            },
            "endpoint": "postnatal"
        }
        response = requests.post("http://127.0.0.1:8002/generate-remarks", json=data)
        ...
        logger.error(
            f"Unexpected error in postnatal complications prediction: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
...
async def get_required_features():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting required features: {str(e)}"
        )
